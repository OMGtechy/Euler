...
constexpr int64_t multiplySequence(const int32_t* const source, const size_t sequenceLength) {
	if (sequenceLength == 0) {
		return 0;
	}

	int64_t product = source[0];
...
constexpr int64_t solution(const size_t numberOfAdjacentDigits) {
	...
	int64_t highestProduct = 0;
	for (size_t i = 0; i < (sizeof digits / sizeof digits[0] - numberOfAdjacentDigits); ++i) {
		highestProduct = std::max(multiplySequence(&digits[i], numberOfAdjacentDigits), highestProduct);
	}
